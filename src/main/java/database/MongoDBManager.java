...
//        initialise all the mongoCollections for the Sensors
    ...
    public <T extends MongoStoreModel> void insertDocumentIntoCollection(T document) {
        MongoCollection<T> collection = database.getCollection(document.getMongoCollectionName(),
                document.getClassObject());
    ...
    public <T extends MongoStoreModel> void insertDocumentsIntoCollection(ArrayList<T> documents) {
        MongoCollection<T> collection = database.getCollection(documents.get(0).getMongoCollectionName(),
                documents.get(0).getClassObject());
    ...
    public ArrayList<ActivFitSensorData> queryForRunningEvent(Date userDate) {
        MongoCursor<ActivFitSensorData> cursor = database.getCollection(ActivFitSensorData.MONGO_COLLECTION_NAME, ActivFitSensorData.class)
    ...
    public int queryForTotalStepsInDay(Date userDate) {
        MongoCursor<ActivitySensorData> cursor = database.getCollection(ActivitySensorData.MONGO_COLLECTION_NAME, ActivitySensorData.class)
...
//            get the next Data
    ...
    public int queryHeartRatesForDay(Date date) {
        MongoCursor<HeartRateSensorData> cursor = database.getCollection(HeartRateSensorData.MONGO_COLLECTION_NAME, HeartRateSensorData.class)
